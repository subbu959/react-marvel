import { BASE_URL, CHARCTERS_BASE_URL, API_KEY,PRIVATE_API_KEY } from '../helper';
...
const getAllCharactersAPI = (offset, success) => {
  let url =
    `${BASE_URL}/${CHARCTERS_BASE_URL}?apikey=${API_KEY}&limit=40&orderBy=-modified&offset=${offset}&hash=${hashgen(API_KEY,PRIVATE_API_KEY)}&ts=${ts}`;
  return fetch(url)
    .then(res => res.json())
    .then((resObj) => {
      try {
...
const getCharacterDetailsAPI = (characterId, success) => {
  ...
  let url =
    `${BASE_URL}/${CHARCTERS_BASE_URL}/${characterId}?apikey=${API_KEY}&hash=${hashgen(API_KEY,PRIVATE_API_KEY)}&ts=${ts}`;
  return fetch(url)
    .then(res => res.json())
    .then((resObj) => {
      try {
